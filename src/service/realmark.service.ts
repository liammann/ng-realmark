...
export class RealMarkService {
    ...
    process(markdown: string): string {
      ...
      if(this.headerLinks){
        ...
        let currentURL = window.location.href.split('#')[0]; // remove current hash 

        let regex = new RegExp(/(<h([1-5]))>(.*)(<\/h\2>)/, 'g');
        var match;
        while ((match = regex.exec(HTMLOutput)) != null) {
          let idName = this.scrubHtml(match[3]);
    ...
    private scrubHtml(value: string) : string{
